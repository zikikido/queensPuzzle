...
# into the GooglePackages folder. Bump VERSION together with the manifest.
VERSION=13.13.0
# Script lives in SetupProjectScript/ — work from the repo root
cd "$(dirname "$0")/.." || exit 1
mkdir -p GooglePackages
for p in app remote-config analytics crashlytics; do
    f="com.google.firebase.$p-$VERSION.tgz"
    echo "downloading $f"
    curl -sfL -o "GooglePackages/$f" "https://dl.google.com/games/registry/unity/com.google.firebase.$p/$f" || { echo "FAILED $f"; exit 1; }
done
echo "done"
