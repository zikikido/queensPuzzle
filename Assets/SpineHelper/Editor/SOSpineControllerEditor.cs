...
namespace qp {
    /// <summary>
    /// Controller inspector, twin of SOFlipbookControllerEditor: 'default' and every 'next' are
    /// picked from the EXISTING states (dropdown, no free typing), and a looping state hides its
    /// next/delay fields. 'spineAnim' draws with Spine's own dropdown (fed by skeletonDataAsset).
    /// </summary>
    [CustomEditor(typeof(SOSpineController))]
    public class SOSpineControllerEditor : Editor {

        public override void OnInspectorGUI() {
            serializedObject.Update();
            EditorGUILayout.PropertyField(serializedObject.FindProperty("skeletonDataAsset"));
            ...
            for (int i = 0; i < statesProp.arraySize; i++) {
                ...
                using (new EditorGUILayout.VerticalScope("box")) {
                    ...
                    EditorGUILayout.PropertyField(s.FindPropertyRelative("spineAnim"));
                    EditorGUILayout.PropertyField(s.FindPropertyRelative("speed"));
                    EditorGUILayout.PropertyField(s.FindPropertyRelative("mix"));
                    ...
                    if (loopProp.boolValue) {    // loop → no next, just the rest between cycles
                        ...
                        if (!string.IsNullOrEmpty(nextProp.stringValue)) {
                            ...
                            EditorGUILayout.PropertyField(s.FindPropertyRelative("nextMix"));
                        }
                    }
                }
            }
