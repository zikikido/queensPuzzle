...
def levels_block(chosen, fail=False, cta=None, skip_tutorial=False):
    return ("/* ==== LEVELS (generated by pick_level.py - do not hand-edit) ==== */\n"
            ...
            "/* ==== END LEVELS ==== */") % (
                ...
                '"%s"' % cta if cta else "null",
                "true" if skip_tutorial else "false")


def variant_name(levels, fail=False, cta=None, skip_tutorial=False):
    ...
    if skip_tutorial:
        name += "-notut"
    if cta:
        name += "-" + re.sub(r"[^a-z0-9]+", "", cta.lower())[:10]
    return name


def write(block, levels, fail=False, cta=None, skip_tutorial=False):
    ...
    path = os.path.join(VARIANTS, "%s.js" % variant_name(levels, fail, cta, skip_tutorial))
...
def main():
    ...
    print("  cta: %s\n" % (args.cta or "Play now (default)"))
    block = levels_block(chosen, args.fail, args.cta, args.skip_tutorial)
    if args.write:
        write(block, args.levels, args.fail, args.cta, args.skip_tutorial)
    else:
        print(block)
