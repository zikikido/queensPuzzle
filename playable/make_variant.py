...
def js_str(s):
    """JS string literal — the hook carries characters like > and ' from the CLI."""
    if not s:
        return "null"
    return '"%s"' % s.replace("\\", "\\\\").replace('"', '\\"')


def levels_block(chosen, fail=False, cta=None, skip_tutorial=False, hook=None,
                 hint=True, hint_delay=3.0):
    return ("/* ==== LEVELS (generated by make_variant.py - do not hand-edit) ==== */\n"
            "var LEVELS=[\n%s\n];\n"
            "var FAIL_MODE=%s;\n"
            "var CTA_TEXT=%s;\n"
            "var SKIP_TUTORIAL=%s;\n"
            ...
            "/* ==== END LEVELS ==== */") % (
                ...
                js_str(cta),
                "true" if skip_tutorial else "false",
                js_str(hook),
                "true" if hint else "false",
                int(round(hint_delay * 1000)))
...
def variant_name(levels, fail=False, cta=None, skip_tutorial=False, hook=None,
                 hint=True, hint_delay=3.0):
    name = "-".join(str(n) for n in levels)
    if fail:
        name += "-fail"
    if skip_tutorial:
        name += "-notut"
    if not hint:
        name += "-nohint"
    elif abs(hint_delay - 3.0) > 0.01:
        name += "-h%gs" % hint_delay
    if cta:
        name += "-" + slug(cta)
    if hook:
        name += "-" + slug(hook)
...
def write(block, levels, fail=False, cta=None, skip_tutorial=False, hook=None,
          hint=True, hint_delay=3.0):
    ...
    path = os.path.join(VARIANTS, "%s.js" % variant_name(levels, fail, cta, skip_tutorial, hook, hint, hint_delay))
...
def main():
    ...
    ap.add_argument("--fail", action="store_true",
                    help="fail mode: 3 bones, wrong puppy costs one, lose popup at zero")
    ap.add_argument("--cta", metavar="TEXT",
                    help='override the button label, e.g. --cta "Free to play"')
    ap.add_argument("--no-tutorial", action="store_true", dest="skip_tutorial",
                    help="skip the 3 guided rule steps; open straight into hinted free play")
    ap.add_argument("--hook", metavar="TEXT",
                    help='campaign hook under the logo, e.g. --hook "IQ > 130 to solve"')
    ap.add_argument("--hint-delay", type=float, metavar="SEC", dest="hint_delay", default=3.0,
                    help="seconds before the Hint button appears (default 3)")
    ap.add_argument("--no-hint", action="store_true", dest="no_hint",
                    help="no Hint button at all (and no auto-hand) — player gets no help")
    ...
    print("  tutorial on level %d only; final win goes to the store" % args.levels[0])
    print("  fail mode: %s" % ("ON - 3 bones, lose popup" if args.fail else "off"))
    print("  tutorial: %s" % ("SKIPPED" if args.skip_tutorial else "on (level 1)"))
    print("  cta: %s" % (args.cta or "Play now (default)"))
    print("  hook: %s" % (args.hook or "none"))
    print("  hint: %s\n" % ("OFF (no help)" if args.no_hint else "after %gs" % args.hint_delay))
    hint = not args.no_hint
    block = levels_block(chosen, args.fail, args.cta, args.skip_tutorial, args.hook, hint, args.hint_delay)
    if args.write:
        write(block, args.levels, args.fail, args.cta, args.skip_tutorial, args.hook, hint, args.hint_delay)
    else:
        print(block)
