...
def write(block, levels, fail=False, cta=None, skip_tutorial=False, hook=None):
    ...
    path = os.path.join(VARIANTS, "%s.js" % variant_name(levels, fail, cta, skip_tutorial, hook))
...
def main():
    ...
    print("  hook: %s\n" % (args.hook or "none"))
    block = levels_block(chosen, args.fail, args.cta, args.skip_tutorial, args.hook)
    if args.write:
        write(block, args.levels, args.fail, args.cta, args.skip_tutorial, args.hook)
    else:
        print(block)
